...
public class FoodDAO {
    ...
    private PreparedStatement ps; //송수신 => SQL 전송 => 결과값을 메모리에 저장
    private static FoodDAO dao; // DAO객체를 한사람당 1개씩만 사용
    // 메모리 공간을 한개만 생성 => 메모리 누수현상 방지
    ...
    public FoodDAO() {
    	try {
    		...
    		// 리플랙션 => 클래스이름 변수, 메소드, 생성자 => 제어 할 수 있게 만든 프로그램
    		// MyBatis / Spring
    		// oracle.jdbc.driver.OracleDriver = DriverManeger
    	}catch(Exception ex) {}
    }
    // 2. 싱글턴 만들기 => 반드시 static : 메모리 공간 1개만 생성
    /*
     *    가비지컬렉션 : 사용하지 않거나 / null 값인 상태의 객체의 메모리 회수
