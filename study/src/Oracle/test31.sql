-- 5장 (프로그램 : 오라클 프로그램, JDBC 프로그램. WEB 프로그램)
/*
       6장 : ER=MODEL : 데이터베이스 설계 => 데이터추출 / 데이터 설계
              키의 종류
       7장 : 정규화
       8장 : 트랜젝션
     ----------------------
       ...
       문법
          1. 변수
                 ...
                 ***5) cursor
                     ...
                     => 라이브러리 제작과 동일 / 메뉴얼
          2. 제어문
                 1) 조건문 : if, if ~ else, if ~ else if ... / case
                 2) 반복문 : 일반 반복문, while / for
          3. 예외처리                 
*/
-- 변수
-- System.out.println() => DBMS_OUTPUT.PUT_LINE()
-- Scanner => & 
-- SELECT => 변수가 아니다 / 화면 출력
SET SERVEROUTPUT ON;
-- 스칼라 변수 (변수명 데이터형)
/*
DECLARE
...
BEGIN
    SELECT empno, ename, job, dname, loc, grade
      ...
      DBMS_OUTPUT.PUT_LINE('부서:'||edg.dname);
      DBMS_OUTPUT.PUT_LINE('근무지:'||edg.loc);
      DBMS_OUTPUT.PUT_LINE('급여:'||edg.grade);
END;
/
*/
-- 스칼라 변수 (변수 데이터형), TYPE변수 (실제 데이터형 읽기)
/*
     형식)
             ------------------------------------ 익명
             DECLARE
...
DECLARE 
   ...
   pTotal NUMBER(7,2):=0;
BEGIN
   SELECT ename, comm, sal, sal_NVL(comm,0)
  ...
  --                   연산 수행이 안된다 
   IF pComm > 0 THEN
      DBMS_OUTPUT.PUT_LINE(pEname||'님의 급여는 '||pSal||'이고 성과급은 '||pComm||'이며 총 급여는 '||pTotal||'입니다');
   ELSE
      DBMS_OUTPUT.PUT_LINE(pEname||'님의 급여는 '||pSal||'이고 성과급은 없고 총 급여는 '||pTotal||'입니다');
   END IF;
END;
/
*/
