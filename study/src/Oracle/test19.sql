-- 데이터 조작언어 (DML => INSERT / UPDATE / DELETE) 187p
/*
     INSERT : 데이터 추가
               = INSERT ALL : 이미 존재하는 테이블에서 전제 데이터를 나눠서 저장
          *** = 전체 데이터 등록
                       - INSERT INTO table_name VALUES(값, 값...) => DEFAULT적용 안됨
                                            ------------- 컬럼과 매칭 => DEFAULT 는 상관없이
               = 원하는 데이터 등록 (NULL 허용 / DEFAULT 가 있는 경우)
...
-- insert => 자동 증가 번호 만들기 SELECT NVL(MAX(hakbun)+1,100
/*
INSERT INTO student(hakbun,name, kor,eng,math)
...
=> SELECT 문장을 이용해서 데이터 저장 가능
=> 많이 사용되는 쿼리는 아니다 (데이터만 재사용)
*/
INSERT INTO student2(hakbun, name, kor, eng, math,refdate)
  SELECT * FROM student;
