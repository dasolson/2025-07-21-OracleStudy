-- TRIGGER : PS/SQL => FUNCTION / PROCEDURE / TIRGGER
/*
      TRIGGER : 특정 이벤트 (INSERT / UPDATE / DELETE)가 발생했을 경우에 자동 저장 프로시저
             1. 특징
                  - 테이블 / 뷰에 연결
                  - 이벤트가 발생하면 자동 호출
                  - INSERT / UPDATE / DELETE 에서만 사용이 가능
                  - 로그파일 / 백업시에 주로 사용
                  - ex) 입고 => 재고 자동화 / 출고 => 재고 자동화
                         *** 맛집 => 찜 => 찜 갯수 증가
                                          --- 맛집 번호 => Function 이용
                  - *** 프로시저 : 댓글 (웹사이트)
              2. 형식
               ...
               3. 시점 : 언제 트리거 처리
                    - BEFORE : DML 실행 전에 동작 => 발생한 테이블 저장 전에 트리거 수행
                    - AFTER   : DML 실행 후에 동작 => 발생한 테이블 저장 후에 트리거 수행
               4. 구분
                    - 행단위 / 문 단위
                            : 각 행마다 실행 : FOR EACH ROW (default)
                               SQL문 전체에 한번만 수행 : 생략
               5. 컬럼
