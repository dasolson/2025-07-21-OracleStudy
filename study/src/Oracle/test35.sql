...
/*
      5장 => PL/SQL : PROCEDURE를 만드는 언어
                              => 제어는 SQL
      ***1. 변수 선언 / 매개변수
             ***1) 스칼라변수 : 데이터형을 임의로 설정
                          no NUMBER, name VARCHAR2(20)
             ***2) TYPE 변수 : 실제 테이블의 컬럼 데이터를 읽기
                          fno menupan_food.fno%TYPE
                          empno emp.empno%TYPE                          
                 3) ROWTYPE : VO와 동일한 개념 => 테이블 컬럼 전체 데이터형을 한번에 읽기
                          emp emp%TYPE
                                 ----- emp가 가지고 있는 모든 컬럼의 데이터형 읽기
                                 사용법 : emp.empno, emp.ename ...
                                 단점    : 1개의 테이블 데이터만 사용이 가능
                 4) RECORD : 사용자 정의 (여러개 테이블을 사용하는 경우)
                                    | 서브쿼리, 조인
                        ...
                        --------------------------------------- 일반 for
                            for-each
                             FOR 변수 IN cursor LOOP => for(MovieVO vo:list)
                                 변수 출력
                        ------------------------------------ 가장 많이 사용
                            for(let vo in 배열) / for(let vo of 배열) / map() / foreach()
                        --------------------------------------------------------------------- 자바스크립트 for문
                형식)
                           선언부
                           구현부
      ...
      3. FUNCTION
              => 리턴형을 가지고 있는 함수 : 내장함수
              => ROW단위 집계 처리
              => SubQuery. JOIN => 복잡한 쿼리가 자주 반복되는 곳
                      CREATE OR REPLACE FUNCTION func_name(매개변수)
               ...
               => 사용 위치
                        SELECT 뒤에 : 컬럼 대신 사용
                        WHERE 뒤에 : 컬럼 대신 사용 | 값으로 사용
                        GROUP BY / ORDER BY
                        => 지원하지 않는 함수 => 사용자 정의 함수
                        => 모든 프로그램은 사용자 정의 (데이터형, 함수, 메소드)가 있기때문에 어렵다
                              | class : record, cursor
                        => 사용자 정의가 없는 프로그램 : HTML / CSS 
                             ------------------------------------------------- publisher               
                        => SELECT 중심         
     ***4. PROCEDURE : 기능만 처리 : DML전체 사용 가능
                                 리턴형이 없다
               형식)
                       CREATE OR REPLACE PROCEDURE pro_name(매개변수...)
                       IS
