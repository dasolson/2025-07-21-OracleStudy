...
/*
        ¿À¶óÅ¬¿¡¼­´Â °´Ã¼
              ...
              => CREATE / DROP
          1. START WITH ------------> ½Ã°¢¹øÈ£
          2. INCREMENT BY ---------> Áõ°¡¹øÈ£
          3. NOCACHE : CACHE ----> ¹Ì¸® »ý¼º (20°³ Á¤µµ)
          4. NOCYCLE : CYCLE ------> Ã³À½ºÎÅÍ ´Ù½Ã ½ÃÀÛ
...
/*
     3Àå
        ...
        DQL : µ¥ÀÌÅÍ °Ë»ö => SELECT
            1) SELECT ÀÇ Çü½Ä , ¼ø¼­
                 ** SELECT¹®Àå => ÄÃ·³ ´ë½Å »ç¿ë °¡´É, Å×ÀÌºí ´ë½Å »ç¿ë °¡´É, Á¶°Ç°ª
                                          -----------------------  ------------------------  --------
                                            ½ºÄ®¶ó¼­ºêÄõ¸®         ÀÎ¶óÀÎºä                 ÀÏ¹Ý¼­ºêÄõ¸®
                  SELECT * | column_list
...
SELECT ename 
FROM emp
WHERE REGEXP_LIKE(ename,'A|B|C|D|E'); => ¼øÂ÷ÀûÀÌÁö ¾ÊÀ»¶§

SELECT ename 
FROM emp
WHERE REGEXP_LIKE(ename,'[A-E]');  => ¼øÂ÷Àû
