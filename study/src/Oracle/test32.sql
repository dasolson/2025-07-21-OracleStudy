...
/*
    형식)
             선언부     DECLARE
             구현부     BEGIN ~ END
             예외 처리 EXCEPTION => 구현부에 포함

             DECLARE
                변수 선언 : 스칼라변수 (일반 변수 = 변수명 데이터형)
...
/*
     형식)
              ...
              *** 반드시 결과값은 1개만 설정
*/
-- SELECT * FROM student; => 단일행 함수 (row단위 처리)
-- JOIN이 있는 경우
...
FROM emp;
-- FUNCTION => 자주 사용하는 부분에서 : ROW단위 집계
SELECT empno, ename, job, hiredate, sal,
