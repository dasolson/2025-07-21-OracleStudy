-- ¿¬»êÀÚ (³í¸®¿¬»êÀÚ)  : µÎ°³ÀÇ Á¶°ÇÀ» ºñ±³ÇÒ °æ¿ì AND, OR
-- AND => Æ÷ÇÔ, µÎ°³ÀÇ Á¶°Ç µ¿½Ã¿¡ true
-- Á÷À§°¡ SALESMANÀÌ°í ±Þ¿©°¡ 1500ÀÌ»óÀÎ »ç¿øÀÇ ¸ðµç Á¤º¸ Ãâ·Â
/*
SELECT *
FROM emp
WHERE job = 'SALESMAN' AND sal >=1500;

-- 81³â¿¡ ÀÔ»çÇÑ ¸ðµç »ç¿øÀÇ Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE hiredate >= '81/01/01' AND hiredate <= '81/12/31';

SELECT *
FROM emp
WHERE hiredate LIKE '81%';

SELECT *
FROM emp
WHERE SUBSTR(hiredate,1,2)='81';

-- OR : µÑÁß¿¡ ÇÑ°³ÀÌ»óÀÌ trueÀÏ¶§ Ã³¸®
-- Á÷À§°¡ MANAGER / CLERKÀÎ »ç¿øÀÇ ¸ðµç Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE job = 'MANAGER' OR job = 'CLERK';
*/
------------------------------------------------------------------------
-- ¿À¶óÅ¬¿¡¼­ Á¦°øÇÏ´Â ¿¬»êÀÚ
-- BETWEEN ~ AND : ±â°£, ¹üÀ§ => ÀÚ¹Ù : ¿¹¾à±â°£, Ã¼Å©ÀÎ±â°£, ÆäÀÌÂ¡...)
-- >= AND <=
/*
- ±Þ¿©°¡ 1000~3000»çÀÌÀÎ »ç¿øÀÇ ¸ðµç Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE sal BETWEEN 1000 AND 3000;

-- 80³âµµ ÀÔ»çÇÑ ¸ðµç »ç¿øÀÇ Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE hiredate BETWEEN '80/01/01' AND '80/12/31';

-- 5°³¸¸ Ãâ·Â
SELECT empno, ename, job, hiredate, rownum
FROM emp
WHERE rownum BETWEEN 1 AND 5;

-- IN => OR°¡ ¿©·¯°³ÀÎ °æ¿ì »ç¿ë
-- deptno(ºÎ¼­¹øÈ£) 10,20,30¹ø Ãâ·Â
SELECT *
FROM emp
WHERE deptno = 10 OR deptno = 20 OR deptno = 30;

-- WHERE ÄÃ·³¸í IN(°ª)
SELECT *
FROM emp
WHERE deptno IN(10,20,30);

-- ¹®ÀÚ -> ´ÙÁß Á¶°Ç °Ë»ö : <imput type=checkbox>, ³¯Â¥(¹®ÀÚ¿­)
-- KING, ADAMS, SCOTT, FORD, MARTIN
SELECT *
FROM emp
WHERE ename = 'KING' OR ename = ' ADAMS' OR ename = 'SCOTT' OR ename = 'FORD' OR ename = 'MARTIN';

SELECT *
FROM emp
WHERE ename IN('KING', 'ADAMS', 'SCOTT', 'FORD', 'MARTIN');

-- NOT(ºÎÁ¤¿¬»êÀÚ)
-- jobÀÌ MANAGER, CLERKÀÌ ¾Æ´Ñ »ç¿øÀÇ ¸ðµç Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE NOT(job='MANAGER' OR job='CLERK');

-- 'KING', 'ADAMS', 'SCOTT', 'FORD', 'MARTIN' »ç¿øÀÌ ¾Æ´Ñ ¸ðµç »ç¿ø Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE ename NOT IN('KING', 'ADAMS', 'SCOTT', 'FORD', 'MARTIN');

-- 81³âµµ¿¡ ÀÔ»çÇÏÁö ¾ÊÀº »ç¿øÀÇ ¸ðµç Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE hiredate NOT BETWEEN '81/01/01' AND '81/12/31';
-- NOT IN, NOT BETWEEN, NOT LIKE

-- NULL => NULL °ªÀÌ ¾ø´Â »óÅÂ => ¿¬»êÃ³¸® ºÒ°¡
-- ¿¬»êÃ³¸®¸¦ À§ÇÑ ¿¬»êÀÚ Á¦°ø => IS NULL, IS NOT NULL
-- »ç¼ö(mgr)°¡ ¾ø´Â »ç¿øÀÇ ¸ðµç Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE mgr IS NULL;

-- »ç¼ö(mgr)°¡ ÀÖ´Â »ç¿øÀÇ ¸ðµç Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE mgr IS NOT NULL;
*/
/*
    LIKE ===> ÁøÈ­ REGEXP_LIKE(ename,'[°¡-ÆR]')
      = % : ¹®ÀÚÀÇ °¹¼ö¸¦ ¸ð¸¦¶§ »ç¿ë(Á¦ÇÑ¾ø´Ù)
      = _  : ÇÑ±ÛÀÚ

     === °Ë»ö

      = ½ÃÀÛ ¹®ÀÚ¿­       : ¹®ÀÚ¿­%   IN%
      = ³¡³ª´Â ¹®ÀÚ¿­    : %¹®ÀÚ¿­   %EN
      = Æ÷ÇÔµÇ´Â ¹®ÀÚ¿­ : %EN%
      = ±ÛÀÚ¼ö¸¦ ¾Ë°í ÀÖ´Â °æ¿ì : _____, __T__
*/
/*
-- »ç¿øÁß¿¡ A ÀÚ·Î ½ÃÀÛÇÏ´Â ¸ðµç »ç¿ø Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE ename LIKE 'A%';

-- TÀÚ·Î ³¡³ª´Â ¸ðµç »ç¿øÀÇ Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE ename LIKE '%T';

-- »ç¿ø ÀÌ¸§Áß¿¡ INÀ¸·Î ³¡³ª°Å³ª ENÀ¸·Î ³¡³ª´Â »ç¿øÀÇ ¸ðµç Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE ename LIKE '%IN' OR ename LIKE '%EN'; 

-- »ç¿ø ÀÌ¸§ Áß¿¡ O¸¦ Æ÷ÇÔÇÏ°í ÀÖ´Â »ç¿øÀÇ ¸ðµç Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE ename LIKE '%O%';

-- »ç¿ø ÀÌ¸§Áß¿¡ A·Î ½ÃÀÛÇÏ°í 5±ÛÀÚÀÎ »ç¿øÀÇ ¸ðµç Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE ename LIKE 'A____';

-- »ç¿øÀÌ¸§ÀÌ 5±ÛÀÚÀÌ°í °¡¿îµ¥°¡ OÀÎ »ç¿øÀÇ ¸ðµç Á¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE ename LIKE '__O__';
*/
-- BOOK => Ãà±¸¸¦ Æ÷ÇÔÇÏ°í ÀÖ´Â Ã¥ Ãâ·Â
SELECT *
FROM book
WHERE bookname LIKE '%Ãà±¸%';

-- name, type => food
-- ÇÑ½ÄÀÌ Æ÷ÇÔµÈ ¸ðµç Á¤º¸ Ãâ·Â
/*
SELECT name,type
FROM food
WHERE type LIKE '%ÇÑ½Ä%';

SELECT name,type
FROM food
WHERE type LIKE '%ºÐ½Ä%';
*/

-- address ¸¶Æ÷¿¡ ÀÖ´Â ¸ÀÁý Ãâ·Â
SELECT name,type
FROM food
WHERE address LIKE '%¸¶Æ÷%';

SELECT name,type
FROM food
WHERE address LIKE '%Á¤¸ª%';

-- emp : »ç¿ø ÀÌ¸§Áß¿¡ A,D,K,E,S ¸¦ Æ÷ÇÔÇÑ »ç¿øÁ¤º¸ Ãâ·Â
SELECT *
FROM emp
WHERE ename LIKE '%A%' OR ename LIKE '%D%' OR ename LIKE '%K%' OR ename LIKE '%E%' OR ename LIKE '%S%';

SELECT *
FROM emp
WHERE REGEXP_LIKE(ename,'A|D|K|E|S');








